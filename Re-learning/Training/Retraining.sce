response_matching = simple_matching;  # use the newer features
active_buttons = 1;
button_codes = 1;
...
response_port_output = false; #determines if response events generate port output associated with those events (no ports outputs for events)
...
#instruction screen
trial {
   trial_duration = forever;      #time to present the instruction until participant push button (first_response)
   trial_type = first_response;
   stimulus_event {
      picture {
         text { caption = "Press the button"; } instruction;#name for text (we can call any layer of trial)
         x = 0; y = 0;
       
      };
      response_active = true;
   };
} instruction_trial; #name for trial after brackets, 
...
begin
   ...
   fl.end_of_file() 
   begin
     txt=txt+f1.get_string()+" ";
   end;
   fl.close(); #escape memory, to prevent out of memory
   return txt;
end;
...
begin
	t.set_caption(txt);
   t.set_max_text_width(1000);#if more then tab 
   t.redraw();#to change caption(text) inside picture
end;
	...
	begin
       ...
       if j==last_word then
        ...
        my_text.redraw();
	     ...
	     my_port_code=relearning_type[phraseID];
       end;
       if j<last_word then
...
loop until
   f.end_of_file() 
begin
   if f.end_of_file() then break; #condition for empty files
   end;
...
information.append("relearn"+tab+"sem"+tab+"id"+tab+"sent"+tab+"RT"+new_string);
